...
cases. You can make hacks only if you solve all versions of this task.
...
· t is a palindrome.
. There exists two strings a and b (possibly empty), such that t = a + b ("+" represents concatenation), and a is prefix of s while b is
suffix of s.

Input
The input consists of multiple test cases. The first line contains a single integer t (1 < t < 10^5), the number of test cases. The next t lines
each describe a test case.
...
Output
For each test case, print the longest string which satisfies the conditions described above. If there exists multiple possible solutions, print
any of them.
